...
class MetricasController:
    ...
    def dados(self):
        ...
        self.view.score_3.setText(f"{round(self.ui.principalController._db.select_score_1, 2)}%")
        if filtro == '':
            self.view.lb_filtro.setText("Filtro Atual: Todos")
            self.view.score_1.setText(f"{round(self.ui.principalController._db.select_score_3(filtro), 2)}%")
            self.view.score_2.setText(f"{round(self.ui.principalController._db.select_score_2(filtro), 2)}%")
        else:
